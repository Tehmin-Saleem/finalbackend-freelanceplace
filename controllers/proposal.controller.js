...
exports.hireProposal = async (req, res) => {
  try {
    const { proposalId } = req.params;  // The proposal ID to be hired
    const proposal = await Proposal.findById(proposalId);
    if (!proposal) {
      return res.status(404).json({ message: "Proposal not found" });
    }

    // Ensure no other proposal is hired for the same job
    const jobProposals = await Proposal.find({ job_id: proposal.job_id });
     console.log("Received proposalId:", proposalId);
  console.log("Received userId:", userId);

    const alreadyHired = jobProposals.some((p) => p.status === "Hired");
    if (alreadyHired) {
      return res
        .status(400)
        .json({ message: "Another proposal for this job is already hired" });
    }

    // Update the selected proposal to "Hired"
    proposal.status = "Hired";
    await proposal.save();

    res.status(200).json({ message: "Proposal hired successfully" });
  } catch (error) {
